...
public class ONoteAttachmentView extends LinearLayout {
    ...
    private List<ODataRow> cursor = new ArrayList<>();
    ...
    public void createView(List<ODataRow> cr) {
        cursor = cr;
        _generateViews();
    }
            ...
            if (items > 0) {
                ...
                for (int i = 0; i < rows; i++) {
                    ...
                    for (int j = 0; j < item_end; j++) {
                        View itemView = mAttachmentViewListener.getView(cursor.get(j),
                                item_index, row);
    ...
    public interface AttachmentViewListener {
        public View getView(ODataRow cr, int position, ViewGroup parent);
    }
}